...
public class SqliteMinTemperatureTable implements DataBaseTable {
    ...
    public SqliteMinTemperatureTable(String url) throws SQLException {
        this.url = url;
    }
    ...
    public void insertIntoTable(String url, Weather weather) {
        final String SQL = "INSERT INTO MinTemperatureTable VALUES(date(?), ?, ?, ?, ?)";
    ...
    public void dropTable(String url) {
        ...
        try {
            ...
            statement.execute("DROP TABLE IF EXISTS MinTemperatureTable;");
        } catch (SQLException ex) {
            Logger.getLogger(SqliteDataBase.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
